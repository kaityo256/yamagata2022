...
さて、この世界は(人間が観測できる範囲では)時間も空間も連続的だが、計算機は離散的な値しか扱うことができない。したがって、計算機の中で「世界」を記述するには、本質的になんらかの離散化が必要となる。さらに、離散化して、この世界をなんらかの数値で表現した時、その数値が何を意味するか、というのは意外に難しい問題だ。
