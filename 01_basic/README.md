...
ここでバネ定数$k$は、数値的には$1.0$と設定した。これがどのくらい「かたい」バネかは、他のパラメータによる。
...
質量$m$や時間$t$が現れるのは左辺だけであるため、$m$を大きくすることは、$t$を小さくすることと等価であることがわかる。例えば、$m=1.0$を、1キログラムではなく、1トンだと思うことにする。それは、質量を1キログラムのまま、時間の$1.0$を1秒ではなく、1000秒だと思った場合と等価だ。つまり、動画をスロー再生すると、質量を大きく感じることになる。実はこれがウルトラマンやゴジラといった特撮で古くから使われている手法だ。

スケーリングについて、きちんと定式化しておこう。数値計算では、値がメートルや秒といった単位を持っているといろいろ不便だし、そもそもあまり意味を持たない。そこで、「数値計算に登場する物理量は、すべて基本的なスケールで無次元化している」と約束する。
