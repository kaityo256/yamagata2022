...
いま、普通のサイコロがあったとしよう。1から6までの目が書いてある。それぞれの目の出る確率が等しい時、サイコロの目の期待値はどれくらいだろうか？答えはすぐに「3.5」だとわかるが、後のために用語を定義しながらきちんと議論しておく。しばらく「当たり前」の議論が続くが、我慢してついてきて欲しい。
...
と期待値が厳密に計算できる。さて、サイコロが公平でない、つまり出る目が偏っており、事前にどの目がどれくらい出るかわからないとしよう。目として$k$が出る確率$p_k$が事前にわからない場合、まずはその確率を推定してやる必要がある。

まずは何度もサイコロを振って、$k$が出た回数を数えよう。これを$w_k$とします。サイコロを振った回数の総数$N$は、$w_k$の和で表せる。
...
を得た。要するに何度もサイコロを振って、その平均を取りなさい、と言ってるだけだ。ただ、

* 期待値とは、値の出現確率$p_k$に値$k$をかけたものの和である
* 出現確率$p_k$が事前にわからないのなら、出現頻度$w_k$を推定し、そこから出現確率$p_k$を推定する必要がある

ということに注意しよう。ここで目として$k$が出る確率は$w_k$に比例している。この$w_k$を「重み」と呼ぶ。事前に重みがわかれば、その総和と重みの比が出現確率を与える。$w_k$の総和を$Z$とすると、
...
となる。公平なサイコロの要件は、全ての目の出現重みが等しいことだ。$w_k$はお互いの比だけが問題となるので、$w_k = 1$としよう。すると、$Z=6$、$p_k = 1/6$となる。我々が「公平なサイコロを振ったら、各目が出る確率が1/6だ」という時には、
...
さて、普通のサイコロでは、全ての目が異なっていたため、$k$番目の状態の目の値$Y_k$は$k$に等しく、単に$k$で置き換えればよかった。次に、目に重複のあるサイコロを考えてみよう。
...
サイコロの例は簡単過ぎたので、もう少し興味ある物理現象の例として気液相転移を考えよう。相転移とは、温度等を変化させた時に、ミクロな性質は変わらないにも関わらず、マクロな性質が大きく変化することだ。例えば、大気圧下において水は100度で沸騰して水蒸気になるが、ミクロにみれば水分子の性質は全く変わっていない。水分子同士の相互作用は温度に依存しないと思われるのに、温度により固体、液体、気体と、全く振る舞いが異なる状態になる。

このうち、気体と液体の相転移を考えてみよう。原子が近距離で斥力、中間距離で引力、遠距離で相互作用しないという条件を満たすと気体と液体の相転移がおきる。これを非常に単純化し、格子上に原子を置く格子ガス模型を考えてみる。
...
w_k = \exp(-\beta E_k)
$$
に比例するとしよう。ここで、$\beta$は$k_B$はボルツマン定数、$T$は絶対温度として$\beta = 1/k_B T$で表される逆温度だ。これは、エネルギーが低い状態ほど出現しやすいことを表現している。
...
では$N=4$の時は？$N=5$は？少し考えれば、$N$が増えるにつれて指数関数的に考えるべき条件が増え、一般の状態数を求めることができないことがわかるであろう。状態数$g_j$がわからないため、重みの総和$Z$も求めることができない。したがって、あるエネルギーを持つ一つの状態の出現確率も求めることができない。
